...
struct Internal {
  ...
  /*--------------------[ SAT-RL minimal control ]---------------------*/

  // Next decision override (0 means no override).
  int rl_next_var;

  // Remaining decisions in the current RL window. Decremented after each
  // regular decision in 'decide()' (i.e., when a decision is assumed),
  // not during pseudo-level assumption/constraint handling.
  int rl_window_left;

  // Nominal window size hint (can be used by external controller).
  int rl_M;

  // Monotonically increasing counter of decisions taken (for logging /
  // synchronization with external controllers).
  uint64_t rl_step_id;
  
  // (removed LBD and custom cumulative counters)

  // SAT-RL Step 2 — observation & candidate snapshot
  // Collect top-K EVSIDS candidates (skip assigned/inactive), returned in parallel arrays.
  void rl_collect_topk (int K, std::vector<int> &out_vars, std::vector<double> &out_scores);

  // Run the inner CDCL loop until the next decision point, then return
  // a compact observation and candidate menu without making a decision.
  //  - K: number of candidates to collect
  //  - timeout_ms: soft cap on work (0 disables)
  //  - out_global: [time, conflicts, decisions, props_per_dec, restarts, 0, 0, 0]
  //  - out_vars/out_scores: up to K pairs (var, evsids)
  //  - out_status: 0/10/20 = unknown/SAT/UNSAT
  void rl_until_hook (int K, int timeout_ms,
