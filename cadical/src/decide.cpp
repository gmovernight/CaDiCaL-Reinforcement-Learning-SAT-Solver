...
#ifdef SATRL_SANITY
  // Test-only: trigger a Top-K snapshot so RLTOPK/RLCAND lines print.
  if (getenv("SATRL_LOG_TOPK")) {
    std::vector<int>    _satrl_vars;
    std::vector<double> _satrl_scores;
    rl_collect_topk(16, _satrl_vars, _satrl_scores); // K=16 for Step 2
  }
#endif
  
#ifdef SATRL_SANITY
  // SAT-RL: allow one-shot manual trigger of the override path via environment variable.
  // On the first call to decide() with SATRL_FORCE_ONCE set, we seed 'rl_next_var'
  // using the solver's own candidate to exercise the override + clearing + window/step asserts.
  {
    static bool satrl_force_injected = false;
    if (!satrl_force_injected && std::getenv("SATRL_FORCE_ONCE")) {
      rl_next_var = next_decision_variable ();
      satrl_force_injected = true;
      std::fprintf(stderr, "c SATRL_SANITY: FORCE_ONCE engaged, step=%llu var=%d\n", (unsigned long long) rl_step_id, rl_next_var);
...
assert (!satisfied ());
  ...
  if ((size_t) level < assumptions.size ()) {
    ...
    if ((size_t) level < assumptions.size () ||
      ...
      stats.decisions++;
#ifdef SATRL_SANITY
      const int sanity_before_window = rl_window_left;
      static long long sanity_prev_step = -1;
#endif
      if (!decision) {
        int idx = 0;
...
#ifdef SATRL_SANITY
    {
      ...
      rl_collect_topk(16, _vars, _scores);  // triggers Top-K asserts
    }
#endif
#ifdef SATRL_SANITY
        const int sanity_forced = rl_next_var;
#endif
        // SAT-RL: if an override is set, honor it once for the very next decision.
        if (rl_next_var) {
          idx = rl_next_var;
          rl_next_var = 0;
        } else {
...
#ifdef SATRL_SANITY
        if (sanity_forced) {
          assert(idx == sanity_forced);
          assert(rl_next_var == 0);
        }
#endif
      ...
      // SAT-RL: update window and step counter
      if (rl_window_left > 0) rl_window_left--;
      rl_step_id++;
#ifdef SATRL_SANITY
      if (sanity_before_window > 0) assert(rl_window_left == sanity_before_window - 1);
      else assert(rl_window_left == 0);
      if (sanity_prev_step >= 0) assert(rl_step_id == sanity_prev_step + 1);
      sanity_prev_step = rl_step_id;
#endif
    }
  }
  if (res)
