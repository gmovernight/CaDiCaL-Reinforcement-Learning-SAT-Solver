...
Internal::Internal ()
    : mode (SEARCH), unsat (false), iterating (false),
      ...
      ext_clause_forgettable (false), tainted_literal (0), rl_next_var (0), rl_window_left (0), rl_M (50), rl_step_id (0), notified (0),
...
// SAT-RL Step 2 — implementation
void Internal::rl_collect_topk (int K, std::vector<int> &out_vars, std::vector<double> &out_scores) {
  out_vars.clear ();
  out_scores.clear ();
  if (K <= 0) return;
  // We scan stab[1..max_var] and keep a local vector then select top-K.
  struct Pair { int v; double s; };
  std::vector<Pair> tmp;
  tmp.reserve ((size_t) max_var);
  for (int idx = 1; idx <= max_var; ++idx) {
    if (!active (idx)) continue;
    if (val (idx)) continue;        // skip assigned
    const double s = stable ? stab[idx] : btab[idx];     // EVSIDS table
    tmp.push_back (Pair{idx, s});
  }
  if (tmp.empty ()) return;

  // If fewer than K, just sort all; otherwise nth_element then sort top-K.
  if ((int) tmp.size () <= K) {
    std::sort (tmp.begin (), tmp.end (), [](const Pair &a, const Pair &b){ return a.s > b.s; });
  } else {
    auto nth = tmp.begin () + K;
    std::nth_element (tmp.begin (), nth, tmp.end (), [](const Pair &a, const Pair &b){ return a.s > b.s; });
    tmp.erase (nth, tmp.end ());
    std::sort (tmp.begin (), tmp.end (), [](const Pair &a, const Pair &b){ return a.s > b.s; });
  }

  for (const auto &p : tmp) {
    out_vars.push_back (p.v);
    out_scores.push_back (p.s);
  }

#ifdef SATRL_SANITY
  if (getenv("SATRL_LOG_TOPK")) {
    static int satrl_printed_topk = 0;
    const int satrl_limit = getenv("SATRL_LOG_LIMIT") ? atoi(getenv("SATRL_LOG_LIMIT")) : 5;
    if (satrl_printed_topk++ < satrl_limit) {
      std::fprintf(stderr, "c RLTOPK K=%d size=%zu\n", K, out_vars.size());
      for (size_t i = 0; i < out_vars.size(); ++i)
        std::fprintf(stderr, "c RLCAND i=%zu var=%d evsids=%.9g\n", i, out_vars[i], out_scores[i]);
    }
  }
#endif
}

// Helper to assemble a small fixed-size global observation.
static inline void satrl_pack_global (Internal *I, std::vector<double> &g) {
  g.clear ();
  double t = absolute_process_time ();
  double conflicts = (double) I->stats.conflicts;
  double decisions = (double) I->stats.decisions;
  double props = (double) I->stats.propagations.search;
  double props_per_dec = decisions > 0.0 ? (props / decisions) : 0.0;
  double restarts = (double) I->stats.restarts;
  g.push_back (t);
  g.push_back (conflicts);
  g.push_back (decisions);
  g.push_back (props_per_dec);
  g.push_back (restarts);
  // Pad to fixed size (8) for stable layouts in pybind.
  g.push_back (0.0);
  g.push_back (0.0);
  g.push_back (0.0);
}
...
#ifdef SATRL_SANITY
// Parse once; return -1 if unset/invalid. Used to delay hook emission.
static long satrl_parse_after_conflicts () {
  static bool inited = false;
  static long val    = -1;              // -1 means "no delay"
  if (!inited) {
    inited = true;
    if (const char *a = std::getenv ("SATRL_HOOK_AFTER_CONFLICTS")) {
      char *e = nullptr;
      long v  = std::strtol (a, &e, 10);
      if (e && *e == '\0' && v > 0) val = v;
    }
  }
  return val;
...
void Internal::rl_until_hook (int K, int timeout_ms,
                              std::vector<double> &out_global,
  ...
  const double t_end = timeout_ms > 0 ? absolute_real_time () + 1e-3 * (double) timeout_ms : 0.0;

  // Mirror assumption-level logic that gates 'decide()' in the main loop.
  int last_assumption_level = assumptions.size ();
  if (constraint.size ()) last_assumption_level++;

  // We drive the inner loop until the point where the main loop would call 'decide()'.
  while (true) {
    if (unsat) { out_status = 20; break; }
    if (unsat_constraint) { out_status = 20; break; }

    if (!propagate ()) {
      // Conflict found, analyze like the normal loop and continue.
      analyze ();
      continue;
    }
    ...
    if (search_limits_hit ()) break;
    if (terminated_asynchronously ()) break;

    // At this point the normal loop would either 'break' for assumptions
    // or proceed to 'decide()'. We stop here "right before decision".
    // The caller can then choose an action and set 'rl_next_var' before resuming.
    if (level >= last_assumption_level) break;

    // Timeout guard (soft).
    if (timeout_ms > 0 && absolute_real_time () >= t_end) break;
  }

  // Assemble observation and candidate menu.
  satrl_pack_global (this, out_global);
  rl_collect_topk (K, out_vars, out_scores);


#ifdef SATRL_SANITY
  const long satrl_need_conf = satrl_parse_after_conflicts ();
  const bool satrl_skip_hook = (satrl_need_conf > 0) &&
                               ((long) stats.conflicts < satrl_need_conf);
#endif

#ifdef SATRL_SANITY
  // Only log when requested AND after the optional conflict threshold.
  if ((std::getenv("SATRL_LOG_HOOK") || std::getenv("STRL_LOG_HOOK")) &&
      !satrl_skip_hook) {
    // Bump global step counter so 'step=' increases each time.
    const long long step = (long long) (rl_step_id++);

    const double      _t  = out_global.size() > 0 ? out_global[0] : 0.0;
    const long long   _c  = out_global.size() > 1 ? (long long) out_global[1] : 0;
    const long long   _d  = out_global.size() > 2 ? (long long) out_global[2] : 0;
    const double     _ppd = out_global.size() > 3 ? out_global[3] : 0.0;
    const long long   _r  = out_global.size() > 4 ? (long long) out_global[4] : 0;

    std::fprintf(stderr,
      "c RLHOOK step=%lld status=%d time=%.6f conflicts=%lld decisions=%lld ppd=%.6f restarts=%lld\n",
      step, out_status, _t, _c, _d, _ppd, _r);
